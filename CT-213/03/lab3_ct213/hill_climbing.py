...
def hill_climbing(cost_function, neighbors, theta0, epsilon, max_iterations):
    ...
    history = [theta0]
    iterations = 0
    better_neighbor = theta0

    while cost_function(theta) > epsilon and iterations < max_iterations:
        for position in neighbors(theta):
            if cost_function(better_neighbor) > cost_function(position):
                better_neighbor = position
        
        if cost_function(better_neighbor) > cost_function(theta):
            return theta, history

        theta = better_neighbor
