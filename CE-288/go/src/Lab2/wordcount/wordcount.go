...
func mapFunc(input []byte) (result []mapreduce.KeyValue) {
	...
	//fmt.Printf("\nMapFunc Input\n") //Para ajudar nos testes. Precisa da biblioteca fmt (acima comentada)
	...
	for _, word := range words {
		...
		newEntry := mapreduce.KeyValue{Key: strings.ToLower(word), Value: "1"}
...
func reduceFunc(input []mapreduce.KeyValue) (result []mapreduce.KeyValue) {
	...
	for key, element := range counter {
		newEntry := mapreduce.KeyValue{Key: key, Value: strconv.Itoa(element)}
		result = append(result, newEntry)
	}
